...
class ResNet(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        x = self.maxpool(x)
        print(x.size())
        #torch.Size([16, 128, 57, 57])

        x = self.layer1(x)
        print(x.size())
        # torch.Size([16, 256, 57, 57])
        x = self.layer2(x)
        print(x.size())
        # torch.Size([16, 512, 29, 29])
        x = self.layer3(x)
        print(x.size())
        # torch.Size([16, 1024, 29, 29])
        ...
        x = self.layer4(x)
        print(x.size())
        # torch.Size([16, 2048, 29, 29])

        x = self.head(x, self.recurrence)
        print(x.size())
        # torch.Size([16, 19, 29, 29])

        outs = [x, x_dsn]
        # print(outs)

        if self.criterion is not None and labels is not None:
            # print("return self.criterion(outs, labels):"+self.criterion(outs, labels).size())
            print("return self.criterion(outs, labels):")
            return self.criterion(outs, labels)
        else:
            # print("return outs:"+outs)
            print("return outs:")
            return outs
