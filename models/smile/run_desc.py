...
from .utils import crop_to_shape, dice_loss_tp2, mse_loss, msge_loss, cost_xentropy_loss, dice_loss,  xentropy_loss
...
def train_step(batch_data, run_info):
    ...
    # # Cost sensitive
    # loss_func_dict = {
    #     "bce": cost_xentropy_loss,  # Cost sensitive
    #     "dice": dice_loss,
    #     "mse": mse_loss,
    #     "msge": msge_loss,
    # }
    ...
    # # Alpha-Dice
    # loss_func_dict = {
    #     "bce": xentropy_loss,  # Cost sensitive
    #     "dice": dice_loss_tp2,  # Alpha-Dice
    #     "mse": mse_loss,
    #     "msge": msge_loss,
    # }

    # Cost sensitive + Alpha-Dice
    loss_func_dict = {
        "bce": cost_xentropy_loss,  # Cost sensitive
        "dice": dice_loss_tp2,  # Alpha-Dice
        "mse": mse_loss,
    ...
    for branch_name in pred_dict.keys():
        ...
        for loss_name, loss_weight in loss_opts[branch_name].items():
            ...
            # print(str(loss_name)+str(loss_weight))
            loss[branch_name] += loss_weight * term_loss
    loss_all = loss["tp"] + loss["np"] +loss["hv"]
    track_value("overall_loss", loss_all.cpu().item())
    # * gradient update

    # torch.set_printoptions(precision=10)
    loss["tp"].backward(retain_graph=True)
    loss["np"].backward(retain_graph=True)
    loss["hv"].backward()
    optimizer.step()
    ####
